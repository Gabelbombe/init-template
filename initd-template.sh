...
case "$1" in
  start)
    if is_running; then
        ...
        if ! is_running ; then
            echo "Unable to start, see $stdout_log and $stderr_log" ; exit 1
        fi
    fi
  ;;
  ...
  stop)
    if is_running; then
        ...
        if is_running ; then
          echo "Not stopped; may still be shutting down or shutdown may have failed" ; exit 1
        else
          echo "Stopped"
          if [ -f "$pid_file" ] ; then
            rm "$pid_file"
  ...
  restart)
    $0 stop
    if is_running ; then
      echo "Unable to stop, will not attempt to start" ; exit 1
    fi
    $0 start
  ;;
